...
interface props {
  favoriteEvent: IFavorite
}

const Card: React.FC<props> = ({ favoriteEvent }: props) => {
  const { photo, name, marvelId } = favoriteEvent;
  ...
  return (
    <div className="bg-primary rounded-md border-black border flex flex-col overflow-hidden relative">
      ...
      />
      <Link to={`events/${marvelId}`} className="text-white absolute top-0 left-0 cursor-pointer">
        <FaSearch
          size={25}
        />
      </Link>
