...
import { ISerie, ILikePayload } from '../state/types';
...
const Card: React.FC<props> = ({ serie, isFavorite }: props) => {
  const { thumbnail: { path, extension }, title: name, id } = serie;
  ...
  return (
    <div className="bg-primary rounded-md border-black border flex flex-col overflow-hidden relative">
      ...
      />
      <Link to={`series/${id}`} className="text-white absolute top-0 left-0 cursor-pointer">
        <FaSearch
          size={25}
        />
      </Link>
