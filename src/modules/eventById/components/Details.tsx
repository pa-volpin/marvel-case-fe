...
const Details: React.FC<RouteComponentProps> = (props) => {
  ...
  const fetchEvent = useCallback((payload: IEvent['data']['id']) =>
    dispatch(fetch(payload)), []);
  
  const event = useSelector((state: IApplicationState) => state.event.data);
  const loading = useSelector((state: IApplicationState) => state.event.loading);
  const error = useSelector((state: IApplicationState) => state.event.error);
  ...
  const buttons = [
    { label: 'Comics', qty: event?.comics?.returned || 0, option: 'comics' },
    { label: 'Characters', qty: event?.characters?.returned || 0, option: 'characters' },
    { label: 'Series', qty: event?.series?.returned || 0, option: 'series' },
    { label: 'Stories', qty: event?.stories?.returned || 0, option: 'stories' },
  ];
  ...
  return (
    <div className="flex flex-col space-y-4 relative h-screen">
      ...
      { loading && <Loading />}
      { error !== '' && !loading && isEmpty(event) && <p>{ `Houve Erro ${error}` }</p> }
      { !loading && !isEmpty(event) && 
          ...
          <div className="bg-primary rounded-md flex flex-col">
            <div className="flex flex-col md:flex-row items-center space-y-2 md:space-x-10">
              <div className="w-full md:w-1/3 h-48 bg-primary rounded-md overflow-hidden">
                <img src={`${event.thumbnail.path}.${event.thumbnail.extension}`} alt={event.title} className="object-fit max-w-full max-h-full h-full w-full" />
              </div>
